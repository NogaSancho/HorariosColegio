...
function MateriasSemestre2({
  ...
  const handleSemesterMaterials = (colorHorario, selectedGradoActual) => {
    ...
    switch (selectedGradoActual) {
      case "8":
        return colorHorario === "verde2"
          ? Object.entries(materias.materias8verde2)
          : Object.entries(materias.materias8azul2);
      case "9":
        return colorHorario === "verde2"
          ? Object.entries(materias.materias9verde2)
          : Object.entries(materias.materias9azul2);
      case "10":
        return colorHorario === "verde2"
          ? Object.entries(materias.materias10verde2)
          : Object.entries(materias.materias10azul2);
      case "11":
        return colorHorario === "verde2"
          ? Object.entries(materias.materias11verde2)
          : Object.entries(materias.materias11azul2);
      default:
        return null;
    }
  };

  const materias10 = handleSemesterMaterials(
    currentColorSemester,
    selectedGradoActual
  );
  ...
  const materiasFiltradas = materias10
    ? materias10.filter(
      ([_, materia]) => materia.letra === currentLetraFiltro || _ === null
    )
    : [];

  console.log(
    `Letra filtro: ${currentLetraFiltro}, colorHorario: ${currentColorSemester}`
  );
        ...
        >
          ...
          <select className="form-select" id="inputGroupSelect02">
            ...
            {materiasFiltradas.map(([id, materia]) => (
              <option key={id} value={materia.letra}>
                {materia.nombre}
              </option>
            ))}
          </select>
          <div className="form-check">
            ...
            <label
              ...
              htmlFor={`checkAguamarinaMateria ${currentLetraFiltro}`}
            >
              Aguamarina
            </label>
          </div>
